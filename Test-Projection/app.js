...
require('now-logs')('miky-demio');

var didExist = fs.existsSync(path.join('tmp', 'invoices.sqlite'));
console.log(__dirname, path.join('/tmp', 'invoices.sqlite'))
sequelize = new Sequelize('sqlite://' + path.join('/tmp', 'invoices.sqlite'), {
  dialect: 'sqlite',
  storage: path.join('/tmp', 'invoices.sqlite')
});
...
app.route('/api/invoices')
  .get(function(req, res) {
    Invoice.findAll({ include: [Customer] }).then(function(invoices) {
      res.json(invoices.map(function(invoice) {
  ...
  .post(function(req, res) {
    var invoice = Invoice.build(_.pick(req.body, ['customer_id', 'discount', 'total', 'invoice_items']));
...
app.route('/api/invoices/:invoice_id')
  ...
  .put(function(req, res) {
    Invoice.findById(req.params.invoice_id).then(function(invoice) {
      invoice.update(_.pick(req.body, ['customer_id', 'discount', 'total', 'invoice_items'])).then(function(invoice) {
        res.json(invoice);
      });
    });
  })
